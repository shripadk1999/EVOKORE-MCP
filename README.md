...
- Open the settings file located at `C:\Program Files\EVOKORE-MCP\config.json`.
- Use a plain text editor like Notepad to change values such as:
  - Server port number
...
- Check the installation folder for error logs in the `logs` directory.
- If the program does not start, try reinstalling using the steps above.
